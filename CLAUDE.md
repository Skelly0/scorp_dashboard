...
12. **Design vocabulary is CSS classes in `global.css`, NOT Tailwind utilities.** `.band`, `.s-card`, `.s-card-pad`, `.s-card-header`, `.kpi-block` (+ `.critical` modifier — wrapper border treatment), `.stat-tile`, `.bar-row`, `.bar` (+ `.crit`/`.good`/`.overflow` modifiers — `.overflow` is a diagonal-stripe overlay for ratios > 1.0), `.overton-row`, `.sit-card` (with `.sev-low`/`.sev-warn`/`.sev-crit` modifiers + `.sit-meter`/`.sit-meter-fill` severity bar), `.crisis-index` (with same `.sev-*` modifiers), `.s-card.critical` (mirrors `.kpi-block.critical`), `.tier`, `.heatmap`, `.tbl`, `.kv`, `.layer-tabs`, `.s-chip`, `.s-tag`, `.faction-bar`, `.spark`. The `s-` prefix is used where the bare name might collide with Tailwind. Theme variables (`--bg-2`, `--accent-soft`, `--good`, `--crit-soft`, etc.) live on `:root[data-theme=…]`; *all three* themes must define every variable for cards to render correctly across themes.
...
17. **Open Graph card is hand-committed and URL-pinned.** `public/og-image.png` is a 1200×630 PNG generated by `scripts/generate_og_image.py` (Pillow + Consolas, no JS toolchain needed). Re-run the script if branding changes; commit the resulting PNG. The `og:*` / `twitter:*` URLs in `index.html` are *absolute* and pinned to `https://skelly0.github.io/scorp_dashboard/` because Discord/social scrapers need fully-qualified URLs — Vite's `base: './'` only relativises script/css refs, not meta tags. If the repo is renamed or hosted elsewhere, update both the meta tags and (if branding changes) regenerate the image.

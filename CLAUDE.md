...
14. **Schema validator splits hard vs soft requirements.** `BASE_REQUIRED_RANGES` are throw-on-missing (structural ranges every extractor needs). `SOFT_OPTIONAL_V3_RANGES` is documentation-only — those are read via `scalar_named()` which returns `None` for missing names, so the dashboard degrades gracefully if the GM removes a tuning knob. Mirrors the `Var_Year` pattern.
